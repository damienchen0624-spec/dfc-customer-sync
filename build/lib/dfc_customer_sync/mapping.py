...
def map_gender(gender: str) -> str:
    ...
    if gender in gender_map.values():
        return gender
...
def map_source(jvdc_source: str) -> str:
    """巨懂车来源 → 大风车 source 中文名。找不到映射则返回"其他"。"""
    name = _JV_SOURCE_TO_NAME.get((jvdc_source or "").strip())
...
def map_source_code_to_name(code: str) -> str:
    """大风车 source CRM code → 中文名。找不到则返回"其他"。"""
    # 函数运行时 SOURCE 已在模块中定义，直接使用
    return {v: k for k, v in SOURCE.items()}.get(code, "其他")
