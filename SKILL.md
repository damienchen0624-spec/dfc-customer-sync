...
│   ├── list_staff.py         # 销售列表获取(自动/手动配置映射)
│   ├── mapping.py            # 字段映射
│   ├── state.py              # 状态管理
...
- `map_source_code_to_name()`:大风车 source CRM code → 中文名
...
- 销售 → 自动匹配（巨懂车"当前跟进人" → 大风车对应销售，失败时回退 owner_id）
...
- `scripts/list_staff.py` - 销售列表获取(自动/手动配置映射)
...
Bug 修复 & 优化：
1. **修复 `list_staff.py` 改用正确的用户 API** — 从订单 API (`rich-man`) 改为 `/userApi/queryUserList.json`，直接查询员工列表，更准确高效
2. **修复 `run_setup` Chrome 进程泄漏** — 使用 try/finally 确保 Chrome 进程在所有路径下都被 terminate
3. **修复 `jvdc_scraper.py` 调试日志不走 logger** — 新增 `logger` 参数，daemon 模式下日志写入日志文件而非 print
4. **清理 `auth.py` 死代码** — 删除未被任何模块调用的 `api_post()` 函数

### v3.6.5

Bug 修复：
1. **修复 `list_staff.py` 无法获取销售列表** — 原来使用的 `queryFindViewRecordPageInfo.json` API 只能查自定义对象（如 customer），不能查员工/销售。改用订单 API（`rich-man.souche.com/orderOperationApi/queryRecordPageInfo.json`）从订单数据的 `order_field_owner` 字段提取销售信息，现在能正确获取 30+ 个销售
2. **增加销售列表获取日志** — 逐页显示获取进度，方便排查问题

### v3.6.4

Bug 修复：
1. **修复 `auth.api_post()` 调用 `sys.exit(1)` 导致守护进程退出** — 改为 `raise Exception()`，让 daemon 的 try/except 能正常捕获
2. **修复 `mapping.py` 缺少 `map_source()` / `map_source_code_to_name()`** — 新增来源映射函数，补全字段映射链
3. **修复 `map_gender("先生")` 返回"未知"** — 增加中文名透传逻辑
4. **修复来源硬编码"懂车帝"** — 改为使用巨懂车实际来源字段，通过 `mapping.map_source()` 映射
5. **修复 `jvdc_scraper.py` 的 `source_keywords` 缺"抖音"等常见来源** — 补全来源关键词，文本回退时能正确提取
6. **修复 `follower_mapping` 只配了一条"薛"导致其他销售匹配不上** — 改为始终自动拉取全量销售 + config 覆盖
7. **修复幽灵记录每轮循环** — `process_leads` 水位线推进 1 分钟，消除 `>=` 幽灵击穿
8. **修复 `list_staff.py` 调用 `auth.api_post()` 走错 API 网关** — 改为直接使用 `dfc_client._http_post()` + `dfc_client.QUERY_URL`（`danube-chord` 网关），自动拉销售列表功能现在正常生效

浏览器弹窗修复：
1. **daemon 改为纯 headless 模式** — 移除 CDP 回退路径，守护进程运行时零弹窗
2. **Windows 专用 headless=new** — Chrome 112+ 新版 headless 反检测更强
3. **平台特定 UA** — Windows/Mac/Linux 分别使用对应的 User-Agent
4. **反检测增强** — 新增 `hardwareConcurrency` 覆盖，降低 headless 被识别概率
5. **清理死代码** — 删除不再使用的 `_start_chrome_cdp()`、`_launch_browser_unlock_keychain()` 函数

文档同步：
1. 版本号统一为 3.6.4（`__init__.py`、`SKILL.md`、`pyproject.toml`）
2. 销售字段描述从"留空"更新为"自动匹配巨懂车跟进人"
3. 项目结构补全 `list_staff.py`
4. `field-mapping.md` 清理 v3.4.0 已删除的废弃枚举字典描述

测试修复：
1. 修复 `FakeRow` 缺少 `query_selector_all()` 导致测试崩溃
2. 修复 `test_dfc_client.py` mock 用 `return_value`（应 `side_effect` raise）
3. 重写 `test_sync_daemon.py` 的 `FakeClient`，对齐实际接口
